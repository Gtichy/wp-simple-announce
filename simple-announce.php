<?php
/*
Plugin Name: WP Simple Announcer
Plugin URI: http://www.ephricon.com/tools/wp-announcer
Description: A simple announcements plugin for placing a message at the top of your screen.
Version: 1.0
Author: Garrett Tichy
Author URI: http://www.ephricon.com/about/team/garrett/
*/
...
function sap_backend_scripts($hook) {
	...
	wp_enqueue_style( 'datepicker-style', SIMPLE_ANNOUNCEMENTS_PATH . 'css/jquery-ui-fresh.css');	 
    wp_enqueue_script( 'announcements', SIMPLE_ANNOUNCEMENTS_PATH . 'js/announce.js', array( 'jquery', 'jquery-ui-datepicker' ) );
}
add_action('admin_enqueue_scripts', 'sap_backend_scripts');

function sap_frontend_scripts() {
	wp_enqueue_style( 'announcements-style', SIMPLE_ANNOUNCEMENTS_PATH . 'css/announce.css');	 
    wp_enqueue_script( 'announcements', SIMPLE_ANNOUNCEMENTS_PATH . 'js/announce.js', array( 'jquery' ) );
    wp_enqueue_script( 'cookies', SIMPLE_ANNOUNCEMENTS_PATH . 'js/jquery.cookie.js', array( 'jquery' ) );    
    wp_enqueue_script( 'cycle', SIMPLE_ANNOUNCEMENTS_PATH . 'js/jquery_cycle_min.js', array( 'jquery' ) );    
}
